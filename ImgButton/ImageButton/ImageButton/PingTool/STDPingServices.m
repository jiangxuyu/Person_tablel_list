...
- (void)st_simplePing:(STSimplePing *)pinger didStartWithAddress:(NSData *)address {
    ...
    if (self.STSimplePingDelegata && [self.STSimplePingDelegata respondsToSelector:@selector(st_simplePing:didStartWithAddress:)]) {
        [self.STSimplePingDelegata st_simplePing:pinger didStartWithAddress:address];
    }
}
...
- (void)st_simplePing:(STSimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber {
    ...
    if (self.STSimplePingDelegata && [self.STSimplePingDelegata respondsToSelector:@selector(st_simplePing:didSendPacket:sequenceNumber:)]) {
        [self.STSimplePingDelegata st_simplePing:pinger didSendPacket:packet sequenceNumber:sequenceNumber];
    }
}
...
- (void)st_simplePing:(STSimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error {
    ...
    if (self.STSimplePingDelegata && [self.STSimplePingDelegata respondsToSelector:@selector(st_simplePing:didFailToSendPacket:sequenceNumber:error:)]) {
        [self.STSimplePingDelegata st_simplePing:pinger didFailToSendPacket:packet sequenceNumber:sequenceNumber error:error];
    }
}
...
//    [self _handlePingItem:pingItem];
    ...
    if (self.STSimplePingDelegata && [self.STSimplePingDelegata respondsToSelector:@selector(st_simplePing:didReceiveUnexpectedPacket:)]) {
        [self.STSimplePingDelegata st_simplePing:pinger didReceiveUnexpectedPacket:packet];
    }
}
...
- (void)st_simplePing:(STSimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet timeToLive:(NSInteger)timeToLive sequenceNumber:(uint16_t)sequenceNumber timeElapsed:(NSTimeInterval)timeElapsed {
    ...
    if (self.STSimplePingDelegata && [self.STSimplePingDelegata respondsToSelector:@selector(st_simplePing:didReceivePingResponsePacket:timeToLive:sequenceNumber:timeElapsed:)]) {
        [self.STSimplePingDelegata st_simplePing:pinger didReceivePingResponsePacket:packet timeToLive:timeToLive sequenceNumber:sequenceNumber timeElapsed:timeElapsed];
    }
}
...
- (void)st_simplePing:(STSimplePing *)pinger didFailWithError:(NSError *)error {
    ...
    if (self.STSimplePingDelegata && [self.STSimplePingDelegata respondsToSelector:@selector(st_simplePing:didFailWithError:)]) {
        [self.STSimplePingDelegata st_simplePing:pinger didFailWithError:error];
    }
}
@end
